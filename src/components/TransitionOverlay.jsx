    ...
    const getVisual = () => {
        ...
        if (imgSrc) {
            if (type === 'door') {
                ...
                // 画像の移動方向（上の階に行く時は上から下に流す、下の階に行く時は下から上に流す）
                ...
                return (
                    ...
                    >
                        ...
                        <motion.img
                            src={imgSrc}
                            style={{ position: 'absolute', width: '250%', height: '300%', objectFit: 'cover', top: '-50%', left: '-75%', filter: 'brightness(0.7)' }}
                            initial={{ y: imgInitialY, rotateY: imgInitialRotateY }}
                            animate={{ y: imgAnimateY, rotateY: imgAnimateRotateY }}
                            transition={{ duration: 8, ease: "easeInOut" }}
                        />

                        {/* Text Overlay Sequence - Scrolling like credits */}
                        <div style={{ position: 'absolute', inset: 0, overflow: 'hidden', display: 'flex', justifyContent: 'center', zIndex: 10 }}>
                            <motion.div
                                initial={{ y: '80vh', opacity: 0 }}
                                animate={{ y: '-40vh', opacity: [0, 1, 1, 0] }}
                                transition={{ duration: 10, ease: "linear", times: [0, 0.1, 0.8, 1] }}
                                style={{
                            ...
                            >
                                {['1F', '2F', '3F', '4F', '5F', '6F', '7F'].includes(String(targetFloorId)) ? (
                                    ...
                                    <>
                                        <div style={{ color: 'white', fontSize: '1.5rem', fontWeight: 'bold', letterSpacing: '0.1em', textShadow: '0 2px 10px rgba(0,0,0,0.8)' }}>
                                            問いを立て、構造を描く。
                                        </div>
                        ...
                        {/* Enter Floor Button - Fixed at the bottom */}
                        <motion.div
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ duration: 1, delay: 5 }}
                            style={{ position: 'absolute', bottom: '15%', zIndex: 20, pointerEvents: 'auto' }}
                        >
                            <button
                                onClick={(e) => {
                                    e.stopPropagation();
